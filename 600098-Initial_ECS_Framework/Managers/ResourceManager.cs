...
namespace OpenGL_Game.Managers
{
    static class ResourceManager
    {
        ...
        {
            foreach (var geometry in geometryDictionary) { geometry.Value.RemoveGeometry(); }
            geometryDictionary.Clear();

            foreach (var texture in textureDictionary) { GL.DeleteTexture(texture.Value); }
            textureDictionary.Clear();

            foreach (var shader in shaderDictionary) { GL.DeleteShader(shader.Value); }
            shaderDictionary.Clear();

            foreach (var audio in audioDictionary) { AL.DeleteBuffer(audio.Value); AL.DeleteSource(audio.Value); }
